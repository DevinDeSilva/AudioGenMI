...
class MelAudioLoaderVoxCeleb(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor, int, int]:
        ...
        ids = self.speaker_id2id_map[self.main_file.iloc[index,1]]
...
SPEAKER_TO_ID = {k:v for v,k in enumerate(sorted(train_data['speaker_id'].unique()))}

print("Train class balance:")
print((train_data['speaker_id'].map(SPEAKER_TO_ID).value_counts()/len(train_data)).sort_index())
print("Validation class balance:")
print((val_data['speaker_id'].map(SPEAKER_TO_ID).value_counts()/len(val_data)).sort_index())

weights = 1.0 / train_data['speaker_id'].map(SPEAKER_TO_ID).value_counts().sort_index()
